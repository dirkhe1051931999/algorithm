...
1. 在优先级队列中，每个元素与“优先级”相关联，具有最高优先级的元素首先出列
2. 元素大小的评判可以通过元素本身的自然顺序（natural ordering），也可以通过构造时传入的比较器
3. 推荐使用 JavaScript 数据结构库--[Buckets](https://github.com/mauriciosantos/Buckets-JS)(Buckets 是一个完整的，经过充分测试和记录的数据结构库，用纯 JavaScript 编写。)
...
6. 区间覆盖问题
   ...
   ```
7. 销售比赛
   1. 假设有偶数天，要求每天必须买一件物品或者卖一件物品，一开始手上没有任何物品，已给出每天物品价格表
   2. `问：计算最大收益`
   3. **第一天必须买，最后一天必须卖，最后一天没有物品**
   4. **除过第一天和最后一天，其他天每次取两天的价格，小的买，大的卖，并且把卖价格放入一个最小堆中**
   5. **比较买的价格和堆顶的价格，比堆顶大，这个价格与堆顶交换，这样就保证了卖的价格总是大于买的价格**
   ```js
   ...
   function sell() {
     ...
     var n = 8;
     // 物品价格表
     var price = [10, 11, 2, 35, 77, 10, 24, 32];
     var buy, sell;
     // 优先队列(堆顶永远是最大的)
     var q = new buckets.PriorityQueue(function(a, b) {
       if (a < b) {
         return -1;
       } else if (a > b) {
         return 1;
     ...
     res -= price[0];
     res += price[n - 1];
     // 非第一天和第二个，每次取两天，小的买，大的卖，把卖的价格放入一个最小堆中中，堆顶永远是最大的值
     for (var i = 1; i <= n - 2; i = i + 2) {
       buy = Math.min(price[i], price[i + 1]);
       sell = Math.max(price[i], price[i + 1]);
       if (q.isEmpty()) {
         // 计算收益
         res = res - buy + sell;
         // 把卖的价格放入堆中
         ...
         // 买的价格比堆顶大，那么这个价格就与堆顶交换，
         if (buy > q.peek()) {
           res = res - 2 * q.peek() + buy + sell;
           q.enqueue(buy);
         } else {
           res = res - buy + sell;
           q.enqueue(sell);
